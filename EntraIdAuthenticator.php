...
class EntraIdAuthenticator extends \ExternalModules\AbstractExternalModule
{
    ...
    const ENTRAID_STATE_SEPARATOR = 'EIASEP';
    const ENTRAID_USERNAME = 'EntraId_Username';
    ...
    public function redcap_every_page_top($project_id)
    {
        ...
        if ( $page === 'manager/project.php' ) {
            echo "<style>label:has(.eid-descriptive){width:100%;}</style>";
            return;
        }
    ...
    public function redcap_survey_page_top(
        ...
        foreach ( $projectSettings["survey"] as $index => $surveyName ) {
            ...
            $this->framework->log('EntraId Authenticator: Handling Survey', [
            ...
            try {
                ...
                $this->framework->log('EntraId Authenticator: Error', [ 'error' => $e->getMessage() ]);
            } finally {
                ...
                if ( $surveyPage === 1 || empty($surveyPage) ) {
                    $this->eidLog('EntraId Authenticator: Survey Auth Succeeded', [
                        "EntraIdAuthenticator_NetId" => $id,
                        "instrument"             => $instrument,
                        "event_id"               => $event_id,
                        "response_id"            => $response_id
                    ]);
                } else {
                    $this->framework->log('EntraId Authenticator: Survey Auth Succeeded', [
                        "EntraIdAuthenticator_NetId" => $id,
                        "instrument"             => $instrument,
                        "event_id"               => $event_id,
                        "response_id"            => $response_id,
    ...
    public function redcap_module_configuration_settings($project_id, $settings)
    {
        ...
        try {
            ...
            $this->framework->log('EntraId Authenticator: Error getting choices', [ 'error' => $e->getMessage() ]);
        } finally {
            return $settings;
        }
    }
    ...
    {
        $settings = [
            'domain'       => urlencode($this->framework->getSystemSetting('entraid-domain') ?? ''),
    ...
    private function handleDashboard($dashboard_hash)
    {
        ...
        if ( $dashboard === null ) {
            $this->framework->log('EntraId Authenticator: Dashboard not found', [ 'dashboard_hash' => $dashboard_hash ]);
            return;
        }
        foreach ( $projectSettings["dashboard"] as $thisDashId ) {
            ...
            $this->framework->log('EntraId Authenticator: Handling Dashboard', [
            ...
            try {
                ...
                // Successful authentication
                $this->eidLog('EntraId Authenticator: Dashboard Auth Succeeded', [
                    "EntraIdAuthenticator_NetId" => $id,
                    "dashboard_hash"         => $dashboard_hash,
                    "dashboard_id"           => $dashboard['dash_id'],
                    "dashboard"              => $dashboard['title']
    ...
    private function handleReport($report_hash)
    {
        ...
        if ( $report === null ) {
            $this->framework->log('EntraId Authenticator: Report not found', [ 'report_hash' => $report_hash ]);
            return;
        }
        ...
        foreach ( $projectSettings["report"] as $thisReportId ) {
            ...
            $this->framework->log('EntraId Authenticator: Handling Report', [
            ...
            try {
                ...
                // Successful authentication
                $this->eidLog('EntraId Authenticator: Report Auth Succeeded', [
                    "EntraIdAuthenticator_NetId" => $id,
                    "report_hash"            => $report_hash,
                    "report_id"              => $report['report_id'],
                    "report"                 => $report['title']
    ...
    private function handleFile ($file_hash) {
        ...
        if ( $file === null ) {
            $this->framework->log('EntraId Authenticator: File not found', [ 'file_hash' => $file_hash ]);
            return;
        }
        ...
        try {
            ...
            // Successful authentication
            $this->eidLog('EntraId Authenticator: File Auth Succeeded', [
                "EntraIdAuthenticator_NetId" => $id,
                "file_hash"            => $file_hash,
    ...
    private function authenticate()
    {
        try {
            ...
            session_start();
            $username = $_SESSION[EntraIdAuthenticator::ENTRAID_USERNAME] ?? '';
            if (!empty($username)) {
                return $username;
            }
            $authenticator = new Authenticator($this);
            $authenticator->handleEntraIDAuth(Utilities::curPageURL());
        } catch ( \Throwable $e ) {
            $this->framework->log('EntraId Authenticator: Error authenticating', [ 'error' => json_encode($e, JSON_PRETTY_PRINT) ]);
            return false;
        }
    }
    ...
    private function eidLog($message, $params = [], $record = null, $event = null)
    {
